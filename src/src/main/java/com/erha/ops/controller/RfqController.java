...
public class RfqController {
    ...
    public ResponseEntity<RFQ> createRfq(@RequestBody RFQ rfq) {
        ...
        try {
            ...
            logger.info("âœ… Created RFQ: {} - {}", created.getJobNo(), created.getDescription());
            return ResponseEntity.status(org.springframework.http.HttpStatus.CREATED).body(created);

        } catch (Exception e) {
            logger.error("âŒ Error creating RFQ: {}", e.getMessage(), e);
    ...
    public ResponseEntity<RFQ> updateRfq(@PathVariable Long id, @RequestBody RFQ rfqDetails) {
        ...
        try {
            ...
            // Update all fields - COMPLETE VERSION
    ...
    public ResponseEntity<RFQ> partialUpdateRfq(@PathVariable Long id, @RequestBody Map<String, Object> updates) {
        ...
        try {
            ...
            if (updates.containsKey("assignedTo")) {
                ...
                logger.info("ðŸ”„ Reassigning RFQ {} to: {}", rfq.getJobNo(), updates.get("assignedTo"));
            }
            ...
            RFQ updatedRfq = rfqRepository.save(rfq);
            logger.info("âœ… Partial update successful for RFQ: {}", updatedRfq.getJobNo());
            return ResponseEntity.ok(updatedRfq);

        } catch (Exception e) {
            logger.error("âŒ Error in partial update: {}", e.getMessage(), e);
            return ResponseEntity.status(org.springframework.http.HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> deleteRfq(@PathVariable Long id) {
        ...
        try {
            ...
            logger.info("âœ… Soft deleted RFQ: {}", rfq.getJobNo());
            ...
            logger.error("âŒ Error deleting RFQ: {}", e.getMessage(), e);
            return ResponseEntity.status(org.springframework.http.HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
