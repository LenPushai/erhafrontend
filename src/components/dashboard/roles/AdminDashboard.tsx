...
const AdminDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const [rfqsRes, quotesRes, jobsRes] = await Promise.all([
        fetch('https://erha-ops-backend-ac4a0f925914.herokuapp.com/api/v1/rfqs').then(r => r.json()).catch(() => ({ content: [] })),
        fetch('https://erha-ops-backend-ac4a0f925914.herokuapp.com/api/v1/quotes').then(r => r.json()).catch(() => []),
        fetch('https://erha-ops-backend-ac4a0f925914.herokuapp.com/api/v1/jobs').then(r => r.json()).catch(() => [])
      ]);
  ...
  const getStatusBadge = (status: string) => {
    const badges: any = {
      new: 'primary', draft: 'secondary', sent: 'info', progress: 'warning',
      approved: 'success', completed: 'success', accepted: 'success', pending: 'warning'
    };
  ...
  return (
      <div>
        {/* Metrics Cards - Clean White with Colored Left Border */}
        <div className="row mb-4">
          {metrics.map((metric, index) => (
              <div key={index} className="col-md-6 col-lg-3 mb-3">
                <div
                    className="card h-100 border-0 shadow-sm"
                    style={{ cursor: 'pointer', borderLeft: `4px solid ${metric.borderColor}` }}
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-start">
                      <div>
                        ...
                        <h2 className="mb-1 fw-bold" style={{ color: metric.textColor }}>{metric.value}</h2>
                        <small className="text-muted">{metric.subtitle}</small>
                      </div>
                      <metric.icon size={32} style={{ color: metric.borderColor, opacity: 0.5 }} />
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* AI Insights Card - Clean White with Blue Border */}
        <div className="row mb-4">
                ...
                <div className="row align-items-center">
                  <div className="col-md-4 mb-3 mb-md-0">
                    <div className="d-flex align-items-center">
                      ...
                      <div>
                        <h5 className="mb-0 text-dark">AI Insights</h5>
                        <small className="text-muted">Powered by PUSH AI</small>
                      </div>
                    </div>
                  </div>
                      ...
                      {aiInsights.map((insight, index) => (
                          <div key={index} className="col-md-6 col-lg-3 mb-2 mb-lg-0">
                            <div className="d-flex align-items-start">
                              ...
                              <div>
                                <small className="d-block text-muted">{insight.label}</small>
                                <strong className="text-dark">{insight.value}</strong>
                              </div>
                            </div>
                          </div>
                      ))}
                ...
                {aiInsights.length > 0 && aiInsights[0].recommendation && (
                    <div className="mt-3 pt-3 border-top">
                      <div className="d-flex align-items-center">
                        ...
                        <small className="text-muted">{aiInsights[0].recommendation}</small>
                      </div>
                    </div>
                )}
              </div>
                ...
                <div className="row align-items-center">
                  <div className="col-md-8">
                    <div className="d-flex align-items-center">
                      ...
                      <div>
                        <h5 className="mb-1 text-dark">Emergency Breakdown Work</h5>
        ...
        <div className="row">
          <div className="col-md-6 mb-4">
            <div className="card border-0 shadow-sm">
              <div className="card-header bg-white border-bottom">
                <h6 className="mb-0 fw-semibold">Financial Overview</h6>
              </div>
              <div className="card-body">
                <div className="row text-center">
                  <div className="col-4">
                    <DollarSign className="text-success mb-2" size={32} />
                    <h5 className="mb-0 fw-bold">{formatCurrencyShort(financials.invoiced)}</h5>
                    <small className="text-muted">Invoiced</small>
                  </div>
                  <div className="col-4">
                    <Clock className="text-warning mb-2" size={32} />
                    <h5 className="mb-0 fw-bold">{formatCurrencyShort(financials.pending)}</h5>
                    <small className="text-muted">Pending</small>
                  </div>
                  <div className="col-4">
                    <TrendingUp className="text-info mb-2" size={32} />
                    <h5 className="mb-0 fw-bold">{formatCurrencyShort(financials.pipeline)}</h5>
