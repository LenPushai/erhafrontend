...
const MainLayout: React.FC = () => {
  ...
  const fetchCounts = async () => {
    try {
      const [rfqs, jobs] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/v1/rfqs`).then(r => r.json()).then(data => Array.isArray(data) ? data : (data.content || [])).catch(() => []),
  ...
  const navItems: NavItem[] = [
    ...
    { type: 'divider', label: 'WORKSHOP' },
    { path: '/workshop', icon: <Kanban size={20} />, label: 'Kanban Board' },
    { path: '/jarison', icon: <Clock size={20} />, label: 'Time Tracking' },
    { path: '/casuals', icon: <HardHat size={20} />, label: 'Casual Labor' },
