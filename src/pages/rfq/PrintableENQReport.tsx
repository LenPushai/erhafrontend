...
const PrintableENQReport: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const rfqRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/v1/rfqs/${id}`);
        setRfq(rfqRes.data);
        
        if (rfqRes.data.clientId) {
          try {
            const clientRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/v1/clients/${rfqRes.data.clientId}`);
