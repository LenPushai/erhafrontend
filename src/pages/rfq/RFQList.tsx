...
const RFQList: React.FC = () => {
  ...
  // Fetch clients and RFQs on mount
  React.useEffect(() => {
    const loadData = async () => {
      // Fetch clients first
      try {
        const clientRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/v1/clients`);
        const clientData = clientRes.data || [];
        const map: Record<number, string> = {};
        clientData.forEach((c: any) => {
          map[c.id] = c.companyName || c.clientCode || `Client ${c.id}`;
        });
        setClientMap(map);
      } catch (err) {
  ...
  const fetchRFQs = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/v1/rfqs`);
  ...
  const getClientName = (clientId: number | null) => {
    ...
    return clientMap[clientId] || `Client ${clientId}`;
  };
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      quoted: statusCounts['Quoted'] || statusCounts['Ready for Quote'] || 0,
  ...
  const getStatusConfig = (status: string) => {
    const configs: { [key: string]: { class: string } } = {
      ...
      'Quoted': { class: 'bg-info text-white' },
      'Converted': { class: 'bg-success text-white' },
  ...
  const filteredAndSortedRfqs = useMemo(() => {
    let filtered = rfqs.filter(rfq => {
      ...
      const clientName = getClientName(rfq.clientId);
    ...
    return filtered;
  }, [rfqs, searchTerm, statusFilter, priorityFilter, entityFilter, sortField, sortDirection, clientMap]);
  ...
  const handleExportToPastel = async (rfqId: number) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/export/pastel/rfqs/${rfqId}`, {
        responseType: 'blob'
      });
  ...
  const handleBulkExport = async () => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/export/pastel/rfqs/bulk`,
  ...
  return (
      <div>
        ...
        <div className="row g-3 mb-4">
          ...
          <div className="col">
            <div className="card border-0 shadow-sm" style={{ borderLeft: '4px solid #17a2b8' }}>
              <div className="card-body py-3">
                <p className="text-muted small mb-1">Quoted</p>
                <h3 className="fw-bold mb-0">{metrics.quoted}</h3>
              </div>
            </div>
          </div>
          <div className="col">
            <div className="card border-0 shadow-sm" style={{ borderLeft: '4px solid #198754' }}>
              <div className="card-body py-3">
                ...
                <h3 className="fw-bold mb-0 text-success">{metrics.converted}</h3>
              </div>
            </div>
          </div>
          <div className="col">
                      ...
                      paginatedRfqs.map((rfq) => {
                        ...
                        return (
                            <tr key={rfq.id}>
                              ...
                              <td className="px-4">
                                <button onClick={() => navigate(`/rfq/${rfq.id}`)} className="btn btn-link text-decoration-none fw-bold p-0">{rfq.jobNo}</button>
                              </td>
                              <td className="px-4 fw-semibold">{getClientName(rfq.clientId)}</td>
                              <td className="px-4 text-muted" style={{ maxWidth: '300px' }}>
                                <div className="text-truncate">{rfq.description}</div>
                              </td>
                              <td className="px-4">
                                <span className={`badge ${statusConfig.class} rounded-pill px-3 py-2`}>{rfq.status}</span>
                              </td>
                              <td className="px-4">
                                <span className={`badge ${priorityConfig.class} rounded-pill px-3 py-2`}>{rfq.priority}</span>
                              </td>
                              <td className="px-4 text-end fw-bold">{rfq.estimatedValue > 0 ? formatCurrency(rfq.estimatedValue) : '-'}</td>
                              <td className="px-4 text-muted">{formatDate(rfq.requestDate)}</td>
                              <td className="px-4">
                                <div className="d-flex justify-content-center gap-1">
                                  <button onClick={() => navigate(`/rfq/${rfq.id}`)} className="btn btn-sm btn-outline-primary">View</button>
                                  <button onClick={() => navigate(`/rfq/${rfq.id}/edit`)} className="btn btn-sm btn-outline-secondary">Edit</button>
                                </div>
                              </td>
                            </tr>
                        );
                    ...
                    <nav>
                      <ul className="pagination pagination-sm mb-0">
                        <li className={`page-item ${currentPage === 1 ? 'disabled' : ''}`}>
                          <button className="page-link" onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1}>Previous</button>
                        </li>
                        <li className="page-item active"><span className="page-link">Page {currentPage} of {totalPages}</span></li>
                        <li className={`page-item ${currentPage === totalPages ? 'disabled' : ''}`}>
                          <button className="page-link" onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages}>Next</button>
                        </li>
                      </ul>
                    </nav>
                  </div>
                        ...
                        <div className="card-body">
                          <div className="d-flex justify-content-between align-items-start mb-3">
                            <div>
                              ...
                              <p className="text-muted small mb-0">{getClientName(rfq.clientId)}</p>
                            </div>
                            <input type="checkbox" className="form-check-input" checked={selectedIds.has(rfq.id)} onChange={() => handleSelectOne(rfq.id)} />
                          </div>

                          <p className="text-muted small mb-3" style={{ minHeight: '40px' }}>
                            {rfq.description && rfq.description.substring(0, 100)}
                            {rfq.description && rfq.description.length > 100 && '...'}
                          </p>
                          ...
                          <div className="border-top pt-3 mb-3">
                            <div className="d-flex justify-content-between mb-2">
                              ...
                              <span className="fw-bold">{rfq.estimatedValue > 0 ? formatCurrency(rfq.estimatedValue) : '-'}</span>
                            </div>
